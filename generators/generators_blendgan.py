...
class ImplicitGenerator3d(nn.Module):
    def __init__(self, siren, neural_renderer, img_size, z_dim, style_dim, f_dim, hidden_dim, **kwargs):
    # def __init__(self, siren, z_dim, hidden_dim, **kwargs):  # siren 512
        super().__init__()
        ...
        self.siren = siren(z_dim=self.z_dim, style_dim=self.style_dim, f_dim=self.f_dim, hidden_dim=self.hidden_dim, device=None)
    ...
    def generate_avg_frequencies(self):
        ...
        z_a = torch.randn((10000, self.z_dim), device=self.siren.device)
        with torch.no_grad():
            frequencies, phase_shifts = self.siren.content_mapping_network1(z_a)
            y_content = self.siren.content_mapping_network2(z_a)
    ...
    #     z_a = torch.randn((10000, self.z_dim), device=self.siren.device)
    #     with torch.no_grad():
    #         frequencies, phase_shifts = self.siren.content_mapping_network(z_a)
    #     self.avg_frequencies = frequencies.mean(0, keepdim=True)
    #     self.avg_phase_shifts = phase_shifts.mean(0, keepdim=True)
    ...
    def forward(self, add_weight_index, z_a, z_b, img_size, fov, ray_start, ray_end, num_steps, h_stddev, v_stddev, h_mean, v_mean, hierarchical_sample, sample_dist=None, lock_view_dependence=False, **kwargs):
        ...
        # 用SIREN模块预测各粗糙采样点的f\sigma
        coarse_output, y = self.siren(add_weight_index, transformed_points, z_a, z_b, ray_directions=transformed_ray_directions_expanded)
        ...
        if hierarchical_sample:
            ...
            # 用SIREN模块预测各精细采样点的RGBsigma
            fine_output, y = self.siren(add_weight_index, fine_points, z_a, z_b, ray_directions=transformed_ray_directions_expanded)
    ...
    #     # 用SIREN模块预测各粗糙采样点的RGB\sigma
    #     coarse_output = self.siren(add_weight_index, transformed_points, z_a, z_b, ray_directions=transformed_ray_directions_expanded).reshape(batch_size, img_size * img_size, num_steps, 4)
    ...
    #         # 用SIREN模块预测各精细采样点的RGBsigma
    #         fine_output = self.siren(add_weight_index, fine_points, z_a, z_b, ray_directions=transformed_ray_directions_expanded).reshape(batch_size, img_size * img_size, -1, 4) # 4表示RGBsigma
    ...
    def staged_forward(self, add_weight_index, z_a, z_b, img_size, fov, ray_start, ray_end, num_steps, h_stddev, v_stddev, h_mean, v_mean, psi=1, lock_view_dependence=False, max_batch_size=70000, depth_map=False, near_clip=0, far_clip=2, sample_dist=None, hierarchical_sample=False, **kwargs):
        ...
        with torch.no_grad():
            raw_content_frequencies, raw_content_phase_shifts = self.siren.content_mapping_network1(z_a)
            raw_y_content = self.siren.content_mapping_network2(z_a)
            if z_b != None:
                raw_style_frequencies, raw_style_phase_shifts = self.siren.style_mapping_network1(z_b)
                raw_y_style = self.siren.style_mapping_network2(z_b)
            else:
            ...
            if hierarchical_sample:
                ...
                for b in range(batch_size):
                    ...
                    while head < fine_points.shape[1]:
                        tail = head + max_batch_size
                        if z_b != None:
                            fine_output[b:b+1, head:tail], _ = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, fine_points[b:b+1, head:tail], truncated_content_frequencies[b:b+1], truncated_content_phase_shifts[b:b+1], raw_style_frequencies[b:b+1], raw_style_phase_shifts[b:b+1], truncated_y_content[b:b+1], raw_y_style[b:b+1], ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail])
                        else:
                            fine_output[b:b+1, head:tail], _ = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, fine_points[b:b+1, head:tail], truncated_content_frequencies[b:b+1], truncated_content_phase_shifts[b:b+1], 0, 0, truncated_y_content[b:b+1], 0, ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail])
                        head += max_batch_size
    ...
    #     with torch.no_grad():
    #         raw_content_frequencies, raw_content_phase_shifts = self.siren.content_mapping_network(z_a)
    #         if z_b != None:
    #             raw_style_frequencies, raw_style_phase_shifts = self.siren.style_mapping_network(z_b)
    #         else:
    ...
    #                 tail = head + max_batch_size
    #                 if z_b != None:
    #                     coarse_output[b:b+1, head:tail] = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, transformed_points[b:b+1, head:tail], truncated_content_frequencies[b:b+1], truncated_content_phase_shifts[b:b+1], raw_style_frequencies[b:b+1], raw_style_phase_shifts[b:b+1], ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail])
    #                 else:
    #                     coarse_output[b:b+1, head:tail] = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, transformed_points[b:b+1, head:tail], truncated_content_frequencies[b:b+1], truncated_content_phase_shifts[b:b+1], 0, 0, ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail])
    #                 head += max_batch_size
    ...
    #                     tail = head + max_batch_size
    #                     if z_b != None:
    #                         fine_output[b:b+1, head:tail] = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, fine_points[b:b+1, head:tail], truncated_content_frequencies[b:b+1], truncated_content_phase_shifts[b:b+1], raw_style_frequencies[b:b+1], raw_style_phase_shifts[b:b+1], ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail])
    #                     else:
    #                         fine_output[b:b+1, head:tail] = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, fine_points[b:b+1, head:tail], truncated_content_frequencies[b:b+1], truncated_content_phase_shifts[b:b+1], 0, 0, ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail])
    #                     head += max_batch_size
    ...
    def forward_with_frequencies(self, add_weight_index, raw_content_frequencies, raw_content_phase_shifts, raw_style_frequencies, raw_style_phase_shifts, img_size, fov, ray_start, ray_end, num_steps, h_stddev, v_stddev, h_mean, v_mean, hierarchical_sample, sample_dist=None, lock_view_dependence=False, **kwargs):
        ...
        if raw_style_frequencies != None:
            coarse_output = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, transformed_points, raw_content_frequencies, raw_content_phase_shifts, raw_style_frequencies, raw_style_phase_shifts, ray_directions=transformed_ray_directions_expanded).reshape(batch_size, img_size * img_size, num_steps, 4)
        else:
            coarse_output = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, transformed_points, raw_content_frequencies, raw_content_phase_shifts, 0, 0, ray_directions=transformed_ray_directions_expanded).reshape(batch_size, img_size * img_size, num_steps, 4)
        ...
        if hierarchical_sample:
            ...
            if raw_style_frequencies != None:
                fine_output = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, fine_points, raw_content_frequencies, raw_content_phase_shifts, raw_style_frequencies, raw_style_phase_shifts, ray_directions=transformed_ray_directions_expanded).reshape(batch_size, img_size * img_size, -1, 4)
            else:
                fine_output = self.siren.forward_with_frequencies_phase_shifts(add_weight_index, fine_points, raw_content_frequencies, raw_content_phase_shifts, 0, 0, ray_directions=transformed_ray_directions_expanded).reshape(batch_size, img_size * img_size, -1, 4)
