...
# def fancy_integration2(f_sigma, f_dim, z_vals, noise_std=0.5, last_back=False, white_back=False, clamp_mode=None, fill_mode=None):
#     """Performs NeRF volumetric rendering."""
#     f = f_sigma[..., :f_dim]                                # [8, 1024, 24, 128]
#     sigmas = f_sigma[..., f_dim:].squeeze()                 # [8, 1024, 24]
#     z_vals = z_vals.squeeze()                               # [8, 1024, 24]

#     deltas = z_vals[..., 1:] - z_vals[..., :-1]             # [8, 1024, 23]
#     delta_inf = 1e10 * torch.ones_like(deltas[..., :1])     # [8, 1024, 1]
#     deltas = torch.cat([deltas, delta_inf], dim=-1)         # [8, 1024, 24]

#     alphas = 1 - torch.exp(-deltas * sigmas)                                                   # [8, 1024, 24]
#     alphas_shifted = torch.cat([torch.ones_like(alphas[:, :, :1]), 1-alphas + 1e-10], dim=-1)  # [8, 1024, 25]
#     weights = (alphas * alphas_shifted[..., :-1]).unsqueeze(-1)        # [8, 1024, 24, 1]
#     weights_sum = weights.sum(2)

#     if last_back:
#         weights[:, :, -1] += (1 - weights_sum)

#     f_final = torch.sum(weights * f, dim=-2)                           # [8, 1024, 128]
#     depth_final = torch.sum(weights * z_vals.unsqueeze(-1), dim=-2)    # [8, 1024, 128]

#     if white_back:
#         f_final = f_final + 1 - weights_sum

#     if fill_mode == 'debug':
#         f_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1., 0, 0], device=f_final.device)
#     elif fill_mode == 'weight':
#         f_final = weights_sum.expand_as(f_final)

#     return f_final, depth_final, weights   # f_final: [8, 1024, 128], weights: [8, 1024, 24, 1]


def fancy_integration2(f_sigma, f_dim, z_vals, noise_std=0.5, last_back=False, white_back=False, clamp_mode=None, fill_mode=None):
    """Performs NeRF volumetric rendering."""
    f = f_sigma[..., :f_dim]                                # [8, 1024, 24, 128]
    sigmas = f_sigma[..., f_dim:]                           # [8, 1024, 24, 1]

    deltas = z_vals[:, :, 1:] - z_vals[:, :, :-1]           # [8, 1024, 23, 1]
    delta_inf = 1e10 * torch.ones_like(deltas[:, :, :1])
    deltas = torch.cat([deltas, delta_inf], -2)             # [8, 1024, 24, 1]

    # noise = torch.randn(sigmas.shape, device='cuda') * noise_std  # 即GRAM提到的nerf_noise
    ...
    alphas = 1 - torch.exp(-deltas * sigmas)                # [8, 1024, 24, 1]

    alphas_shifted = torch.cat([torch.ones_like(alphas[:, :, :1]), 1-alphas + 1e-10], -2)    # [8, 1024, 25, 1]
    # weights = alphas * torch.cumprod(alphas_shifted, -1)[:, :, :-1]                        # [8, 1024, 24, 1]
    ...
    return f_final, depth_final, weights  # f_final: [8, 1024, 128]
...
def fancy_integration(rgb_sigma, z_vals, device, noise_std=0.5, last_back=False, white_back=False, clamp_mode=None, fill_mode=None):
    ...
    # noise = torch.randn(sigmas.shape, device=device) * noise_std  # 即GRAM提到的nerf_noise
...
# 生成相机坐标系下的z_vals和ray directions
def get_initial_rays_trig(n, num_steps, device, fov, resolution, ray_start, ray_end):
    ...
    # rays_d_cam是从相机光心投射出去的单位光线向量
    rays_d_cam = normalize_vecs(torch.stack([x, y, z], -1))
    # 根据采样始末位置构造每条光线上采样点距离光心的绝对距离
...
# 相机坐标系到世界坐标系的映射
def transform_sampled_points(points, z_vals, ray_directions, device, h_stddev=1, v_stddev=1, h_mean=math.pi * 0.5, v_mean=math.pi * 0.5, mode='normal', randomize=False):
    ...
    # 采样相机位姿（光心位置, phi, theta，在世界坐标系中的表示）
